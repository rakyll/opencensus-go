...
func main() {
	...
	videoSize, err := stats.NewMeasureFloat64("/mycompany.com/measure/float64/video_size", "size of processed videos", "MBy")
	if err != nil {
		log.Fatalf("Video size measure not created: %v\n", err)
	}
	videoSpamCount, err := stats.NewMeasureInt64("/mycompany.com/measure/int64/video_spam_count", "number of videos marked as spam", "1")
	if err != nil {
